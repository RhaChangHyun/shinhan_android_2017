...
public class MainActivity extends AppCompatActivity {
    ...
    public void onButtonClicked(View view) {
        ...
        if (!wordString.isEmpty() && !definitionString.isEmpty()) { //단어,뜻을 입력했으면 저장하고 아니면 저장안한다
            writeDatabase(wordString, definitionString);//DB에 저장
            readDatabase();//DB내용 읽기

        }
    }
    public void onDButtonClicked(View view) {

        }


    public void writeDatabase(String word, String definition) {
        Log.i("!!!!!!!!!!!!!!!!!!",word+"/"+definition);
